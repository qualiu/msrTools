...
check_tool() {
    ...
    if [ -f "$toolPath" ]; then
        ...
        toolSaveName=$toolName$toolExtension
        toolPath=~/$toolSaveName
        if [ ! -f ~/$toolSaveName ]; then
            wget "https://github.com/qualiu/msr/blob/master/tools/$toolSaveName?raw=true" -O $toolPath.tmp \
                && mv -f $toolPath.tmp $toolPath \
            || exit_error "Failed to download $toolSaveName"
        fi
    fi
    if [ ! -x $toolPath ]; then
        chmod +x $toolPath
