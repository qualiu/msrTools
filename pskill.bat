...
if !IsAllNumbersAsPIDs! GTR 0 (
    for /f "tokens=*" %%a in ('echo %* ^| msr -t "\s+(\d+)" -o ",\1" -aPAC ^| msr -t "\s+$" -o "" -aPAC') do (
        call !PwshExe! -Command "& '!PsToolPath!' -ProcessIds '%%a' -Action Stop"
    )
    exit /b !ERRORLEVEL!
)
...
:reset_all_args
    ...
    set needQuotes=0
    echo !currentArg!| findstr " " >nul 2>nul && set "needQuotes=1"
    if !needQuotes! EQU 0 (  echo !currentArg!| findstr /R "[^0-9a-zA-Z_.+:,-]" >nul 2>nul && set "needQuotes=1" )
    if !needQuotes! EQU 1 (
