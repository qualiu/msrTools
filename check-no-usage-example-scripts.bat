...
(for /f "tokens=*" %%a in ('msr -rp %~dp0 --nd \.git -f "\.(bat|cmd)$" --nf "check-no-usage-example-scripts.bat|unit-test-before-commit-zen.bat" -l -PAC') do @msr -p %%a -it "^\s*echo.*(Usage|Example)" -l -H 0) | msr -it "^Matched 0 lines.*?Source = (\S+.+?)\s*\.?\s*$" -o "$1" -PI -O -c Check batch scripts which has no usage and examples
