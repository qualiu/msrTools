...
:Download_File
    ...
    if !ERRORLEVEL! EQU 0 (
        wget https://github.com/qualiu/msr/raw/master/tools/%name% -O %name%.tmp --no-check-certificate -q && move /y %name%.tmp %name% && icacls %name% /grant %USERNAME%:RX && move %name% !SaveFolder!\
    ) else (
