...
if ($Action -eq "Stop") {
    if (-not (Test-HasSearchCriteria)) {
        Write-Error "For Stop action, at least one of the following parameters must be specified: ProcessName, CommandLine, ProcessNamePattern, CommandLinePattern, or ProcessIds"
        exit 1
    }
}
